...
<div class="wt_mgdp_history_page">
	<h2 class="wp-heading-inline"><?php esc_html_e('Logs','wp-migration-duplicator');?></h2>
	<p>
		<?php esc_html_e('Lists the developer logs required for debugging purposes. You can share the downloaded log files with the support team in case of issues.','wp-migration-duplicator');?>
	</p>
	...
	if(is_array($log_files) && count($log_files)>0)
	{
		...
		<thead>
			<tr>
				<th class="log_file_name_col"><?php esc_html_e("File",'wp-migration-duplicator'); ?></th>
	...
	{
		?>
		<h4 class="wt_mgdp_history_no_records"><?php esc_html_e("No logs found.",'wp-migration-duplicator'); ?></h4>
		<?php
	}
	?>
</div>