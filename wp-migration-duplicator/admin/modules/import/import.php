...
class Wp_Migration_Duplicator_Import
{
    ...
    public function restore_notice($backup_list, $offset)
    {
        ...
        <div class="wt_warn_box">
            <?php esc_html_e('Restoring the backups will overwrite all files in the system, including existing backups. This action will take you to the system state.', 'wp-migration-duplicator'); ?>
            <br />
            <?php esc_html_e('Do not restore unless you are sure about what you are doing.', 'wp-migration-duplicator'); ?>
        </div>
        <?php
    }
    ...
    public function restore_backup_btn($backup, $file_exists, $file_url)
    {
        if ($file_exists && $backup['status'] == Wp_Migration_Duplicator::$status_complete) {
            ?>
            <button data-file-url="<?php echo esc_attr($file_url); ?>" data-id="<?php echo esc_attr($backup['id_wtmgdp_log']); ?>"
                title="<?php esc_attr_e('Restore', 'wp-migration-duplicator'); ?>"
                class="button button-secondary wt_mgdp_restore_backup" style=""><span class="dashicons dashicons-backup"
                    style="margin-top:4px; color: black"></span></button>
            <?php
        }
            ...
            <script type="text/javascript">
                jQuery(document).ready(function () {
                    jQuery('.wt_mgdp_restore_backup').unbind('click').click(function () {
                        if (confirm('<?php esc_attr_e('Are you sure?', 'wp-migration-duplicator'); ?>')) {
                            var file_url = jQuery(this).attr('data-file-url');
                            if (jQuery.trim(file_url) == "") {
                                alert('<?php esc_attr_e('Error', 'wp-migration-duplicator'); ?>');
                                return false;
                            }
    ...
    /**
     * Recursively merge $src directory (or file) into $dest, moving files.
     * Used to flatten a wrapper folder from a zip without clobbering existing folders.
     */
    function rrmerge($src, $dest)
    {
        if (is_dir($src)) {
            if (!is_dir($dest)) {
                mkdir($dest, 0755, true); // phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_mkdir
            }
            foreach (scandir($src) as $item) {
                if ($item === '.' || $item === '..') {
                    continue;
                }
                $this->rrmerge($src . '/' . $item, $dest . '/' . $item);
            }
            rmdir($src); // phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_rmdir
        } else {
            rename($src, $dest); // phpcs:ignore WordPress.WP.AlternativeFunctions.rename_rename
        }
    }
    ...
    private function start_import($out)
    {
        ...
        if ($archive->extract(PCLZIP_OPT_PATH, $extract_to) == 0) {
            ...
            /**
             * Some zips (esp. when re-packaged/downloaded) wrap everything inside
             * an extra top-level folder, usually named after the zip file, e.g.
             * extract_to/yourbackup/migrator_database/*, extract_to/yourbackup/plugins/*, etc.
             * Detect that and flatten it so the real content (migrator_database, plugins,
             * themes, uploads, mu-plugins) ends up directly under wp-content, where the
             * rest of the import pipeline (and WordPress itself) expects it.
             *
             * NOTE: detection must not rely on migrator_database alone - a files-only
             * export (no DB at all) can still be wrapped, so we check for ANY of the
             * known top-level content folders.
             */
            $known_content_dirs = array('migrator_database', 'plugins', 'themes', 'uploads', 'mu-plugins');

            $has_content_at_root = false;
            foreach ($known_content_dirs as $known_dir) {
                if (is_dir($extract_to . '/' . $known_dir)) {
                    $has_content_at_root = true;
                    break;
                }
            }

            if (!$has_content_at_root) {
                $zip_basename = basename($real_url, '.zip');
                $possible_wrappers = array_unique(array_filter(array(
                    $extract_to . '/' . $zip_basename,
                )));
                // also check any single top-level folder that itself contains a known content folder
                foreach (glob($extract_to . '/*', GLOB_ONLYDIR) as $top_dir) {
                    foreach ($known_content_dirs as $known_dir) {
                        if (is_dir($top_dir . '/' . $known_dir)) {
                            $possible_wrappers[] = $top_dir;
                            break;
                        }
                    }
                }
                $possible_wrappers = array_unique($possible_wrappers);

                foreach ($possible_wrappers as $wrapper_dir) {
                    if (!is_dir($wrapper_dir)) {
                        continue;
                    }
                    $wrapper_has_content = false;
                    foreach ($known_content_dirs as $known_dir) {
                        if (is_dir($wrapper_dir . '/' . $known_dir)) {
                            $wrapper_has_content = true;
                            break;
                        }
                    }
                    if ($wrapper_has_content) {
                        Webtoffe_logger::write_log('Import', "Wrapper folder detected at $wrapper_dir. Flattening into $extract_to.");
                        foreach (scandir($wrapper_dir) as $item) {
                            if ($item === '.' || $item === '..') {
                                continue;
                            }
                            $src = $wrapper_dir . '/' . $item;
                            $dest = $extract_to . '/' . $item;
                            if (file_exists($dest)) {
                                // destination already exists (e.g. plugins/themes folder) - merge recursively
                                $this->rrmerge($src, $dest);
                            } else {
                                rename($src, $dest); // phpcs:ignore WordPress.WP.AlternativeFunctions.rename_rename
                            }
                        }
                        $this->rrmdir($wrapper_dir);
                        break;
                    }
                }
            }
    ...
    private function merge_db($out)
    {
        ...
        if (is_dir($db_path)) {
            ...
            Webtoffe_logger::write_log('Import', "No 'migrator_database' folder found at $db_path — proceeding without a database import. This is expected for files-only backups. If you expected the database to be imported, confirm the backup zip actually contains a database export and that its folder is named 'migrator_database'.");
        }
    ...
    function resumableUpload($tmp_file_path, $filename, $post_data)
    {
        ...
        if (
            count($errors) == 0 and $new_path = $this->checkAllParts(
                $file_chunks_folder,
                $filename,
                $extension,
                $totalSize,
    ...
    function checkAllParts(
        ...
        if (count($parts) == $totalChunks) {
            ...
            if (
                $loaded_size >= $totalSize and $new_path = $this->createFileFromChunks(
                    $file_chunks_folder,
                    $filename,
                    $extension,
                    $totalSize,
